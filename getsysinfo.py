...
def cleanEntry(entry):
    ...
    cleaned_entry = (
        str(entry).strip("['\"]")
    ).strip()  # Remove leading and trailing whitespaces and Python-related [] and '
    cleaned_entry = " ".join(
        cleaned_entry.split()
    )  # Remove consecutive whitespace characters (to make output smaller onscreen)
...
def writeOutfile(line):
    """Write the system information output file to SYS_INFO.txt."""
